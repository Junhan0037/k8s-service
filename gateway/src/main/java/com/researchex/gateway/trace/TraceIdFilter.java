...
public class TraceIdFilter implements GlobalFilter, Ordered {
    ...
    public static final String TRACE_ID_ATTRIBUTE = "researchex.traceId";
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        String providedTraceId = exchange.getRequest().getHeaders().getFirst(TRACE_ID_HEADER);
        String traceId = StringUtils.hasText(providedTraceId) ? providedTraceId.trim() : traceIdGenerator.generate();

        // 다운스트림 서비스로 동일한 TraceId를 전달하기 위해 요청 헤더를 갱신한다.
