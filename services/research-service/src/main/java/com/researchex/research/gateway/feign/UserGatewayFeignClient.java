 ...
 * URL 및 타임아웃은 {@link UserGatewayFeignConfiguration} 에서 주입한 프로퍼티를 따른다.
 */
@FeignClient(
    name = "userGatewayClient",
    url = "${researchex.gateway.user.base-url}",
    configuration = UserGatewayFeignConfiguration.class)
public interface UserGatewayFeignClient {

  @GetMapping("/internal/users/{userId}")
  UserProfileResponse fetchUserProfile(@PathVariable("userId") String userId);
}
