...
class ServiceMetadataControllerTest @Autowired constructor(
    ...
    fun metadataEndpointReturnsServicePayload() {
    ...
    fun metadataEndpointUsesCache() {
        val firstResponse = mockMvc.perform(get("/api/v1/metadata"))
            .andReturn()
            .response
            .contentAsString
        val secondResponse = mockMvc.perform(get("/api/v1/metadata"))
            .andReturn()
            .response
            .contentAsString

        val first = objectMapper.readValue(firstResponse, ServiceMetadataResponse::class.java)
        val second = objectMapper.readValue(secondResponse, ServiceMetadataResponse::class.java)
