...
/**
 * [ResearchQueryController]의 파라미터 파싱과 서비스 위임을 검증하는 WebMvc 슬라이스 테스트.
...
class ResearchQueryControllerWebMvcTest {
    ...
    @Test
    fun searchEndpointParsesQueryParametersCorrectly() {
        val response = ResearchQueryResponse(
            queryId = "query-xyz",
            status = ResearchQueryStatus.COMPLETED,
            items = emptyList(),
            pagination = PaginationMetadata.from(0, 15, 0),
            facets = emptyMap()
        )
        `when`(researchQueryService.search(any(ResearchQueryCriteria::class.java))).thenReturn(response)
        ...
        val criteria = criteriaCaptor.value
        assertThat(criteria.tenantId).isEqualTo("tenant-1")
        assertThat(criteria.query).isEqualTo("cancer")
        assertThat(criteria.page).isEqualTo(1)
        assertThat(criteria.size).isEqualTo(15)
        assertThat(criteria.useCache).isFalse()
        assertThat(criteria.filters["status"]).containsExactly("RECRUITING", "ACTIVE")
        assertThat(criteria.filters["phase"]).containsExactly("PHASE_1")
        assertThat(criteria.sorts).hasSize(2)
    }
    ...
    class TestConfig {
        @Bean
        fun searchProperties(): SearchProperties = SearchProperties()
    }
}
